...
const TOOLS = [
  ...
  { id: 'cartadigital-admin', label: 'Admin Carta Digital', icon: Lock, color: '#ec4899', description: 'Panel de administración de la Carta Digital', url: 'https://migusto.com.ar/cartadigital/admin' },
  { id: 'capacitaciones-admin', label: 'Admin Capacitaciones', icon: GraduationCap, color: '#10b981', description: 'Panel de administración de Capacitaciones', url: 'https://migusto.com.ar/fabrica/capacitaciones/admin' },
];
...
const RegsApp = () => {
  ...
  const [stockResponsable, setStockResponsable] = useState('');
  const [stockTurno, setStockTurno] = useState(() => new Date().getHours() < 13 ? 'Mañana' : 'Tarde');
  const [stockControlDate, setStockControlDate] = useState(getCurrentDate());
  const [historyShiftFilter, setHistoryShiftFilter] = useState('Mañana');
  ...
  const [selectedHistoryDate, setSelectedHistoryDate] = useState(getFormattedToday());
  const [historyFilterDate, setHistoryFilterDate] = useState('');
  const [copyEmailManana, setCopyEmailManana] = useState(true);
  const [copyEmailTarde, setCopyEmailTarde] = useState(true);
  const [pedidoFecha, setPedidoFecha] = useState(getFormattedToday());
  const [pedidoItems, setPedidoItems] = useState([{ producto: '', cantidad: '', unidad: 'KG' }]);
  const [despachoError, setDespachoError] = useState(false);
  const [openStockMenuId, setOpenStockMenuId] = useState(null);
  ...
  const normalizeNotif = (n) => ({
    ...
    emitterName: n.target_sector_name ?? n.targetSectorName ?? n.emitterName,
    refId: n.ref_id ?? n.refId,
    // n.seen es false si se seteo como false o null/undefined
    seen: n.seen === true ? true : false,
  });
  ...
  useEffect(() => {
    const isDetailedStockOpen = selectedRecord && (selectedRecord.type === 'pedido_fabrica' || selectedRecord.tipo === 'pedido_fabrica');
    const rootEl = document.getElementById('root');
    if (rootEl) {
      if (isDetailedStockOpen) {
        rootEl.style.maxWidth = '1600px';
      } else {
  ...
  useEffect(() => {
    ...
    const firstTabMap = {
      ...
      'picadillo': 'pedido-materiales',
      'armado': 'pedido-materiales',
      'salsas': 'pedido-materiales',
      'compras': 'stock-mercaderia-history'
    };
  ...
  // Helper: get notifications for the current sector
  const filteredNotifications = notifications.filter(n => {
    if (!activeSector) return false;
    const currentSectorLower = activeSector.toLowerCase().trim();
    const target = (n.targetSector || n.target_sector || '').toLowerCase().trim();
    
    // Si el destino es 'all', aplica a todos los sectores
    if (target === 'all') return true;

    // Coincidencia exacta o por subcadena/sector
    const isTargetMatch = target === currentSectorLower || target.includes(currentSectorLower) || currentSectorLower.includes(target);
  ...
  const markAllAsSeen = async () => {
    ...
    setNotifications(prev =>
      prev.map(n => {
        const target = (n.targetSector || n.target_sector || '').toLowerCase().trim();
        return (target === currentSectorLower || target === 'all') ? { ...n, seen: true } : n;
      })
    );
    ...
    await supabase
      .from('notificaciones')
      .update({ seen: true })
      .or(`target_sector.ilike.${activeSector},target_sector.eq.all`)
  ...
  const handleGeneratePDF = async (record, action = 'save') => {
    ...
    else if (rType === 'pedido_fabrica') typeLabel = "Stock de Mercadería";
  ...
  const copyStockTableForEmail = () => {
    ...
    let html = `
      <div style="font-family: Arial, sans-serif; color: #000; max-width: 1000px;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; border: 2px solid #000;">
          <thead>
            <tr style="background-color: #000; color: #fff;">
              <th style="width: 30px; padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">#</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">TIPO</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: left; font-weight: 800;">MATERIA PRIMA</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">ALMACENAMIENTO</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">STOCK MINIMO</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">STOCK</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">UNIDAD DE MEDIDA</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">FECHA VENCIMIENTO</th>
              <th style="padding: 10px; border: 1px solid #333; text-align: center; font-weight: 800;">ESTADO</th>
            </tr>
          </thead>
          <tbody>
    `;
    ...
    filteredStocks.forEach((item, idx) => {
      ...
      let stockColor = '#000';
      if (isCrit) {
        stockBg = 'background-color: #ff0000; color: #ffffff;';
      } else if (isLow) {
        stockBg = 'background-color: #ffcc00; color: #000000;';
      } else if (isOk) {
        stockBg = 'background-color: #92d050; color: #000000;';
      }

      const numBg = typeColors[item.tipo.toLowerCase()] || '#f9f9f9';

      let stateColor = '#548235';
      let stateText = item.estado || 'OK VTO';
      if (stateText === 'VENCIDO') {
        stateColor = '#ff0000';
      } else if (stateText === 'POR VENCER') {
        stateColor = '#f59e0b';
      } else {
      ...
      html += `
        <tr>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center; font-size: 11px; color: #555; background-color: ${numBg}; font-weight: 400;">${idx + 1}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center; font-weight: 800; text-transform: uppercase;">${item.tipo}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: left; font-weight: 800; text-transform: uppercase;">${item.producto}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center; text-transform: uppercase;">${item.almacenamiento || ''}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center;">${item.min}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center; font-weight: 950; ${stockBg}">${item.stock}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center; text-transform: uppercase;">${item.unidad}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center;">${item.vencimiento || 'n/a'}</td>
          <td style="padding: 8px; border: 1px solid #ccc; text-align: center; font-weight: 900; color: ${stateColor}; font-size: 12px;">${stateText}</td>
        </tr>
      `;
    });
    ...
    const plainText = filteredStocks.map(item => `${item.tipo} | ${item.producto} | Stock: ${item.stock} (Min: ${item.min}) ${item.unidad}`).join('\n');
    copyToClipboardHTML(html, plainText);
  };

    const handleStockConfirmCarga = () => {
    if (!stockResponsable.trim()) {
      alert('Por favor, ingrese el nombre del responsable de carga.');
      return;
    }
    setConfirmModal({
      show: true,
      title: '¿Confirmar carga de stock a Proveedores?',
      action: async () => {
        const targetDate = formatInputDate(stockControlDate);
        const shiftKey = stockTurno === 'Mañana' ? 'manana' : 'tarde';
        
        const { data: existingRecs } = await supabase
          .from('registros')
          .select('*')
          .eq('sector', 'proveedores')
          .eq('tipo', 'pedido_fabrica')
          .eq('fecha', targetDate);

        const existingRecord = existingRecs && existingRecs.length > 0 ? existingRecs[0] : null;

        let result;
        if (existingRecord) {
          const currentDatos = existingRecord.datos || {};
          const updatedDatos = {
            ...currentDatos,
            [shiftKey]: {
              stockList: stockList,
              responsable: stockResponsable.trim(),
              fechaCarga: targetDate
            }
          };

          const shiftsLoaded = [];
          if (updatedDatos.manana) shiftsLoaded.push('Mañana');
          if (updatedDatos.tarde) shiftsLoaded.push('Tarde');
          const productTitle = `Stock de Mercadería - Turnos: ${shiftsLoaded.join(' y ')}`;
          ...
          const initialDatos = {
            manana: stockTurno === 'Mañana' ? { stockList, responsable: stockResponsable.trim(), fechaCarga: targetDate } : null,
            tarde: stockTurno === 'Tarde' ? { stockList, responsable: stockResponsable.trim(), fechaCarga: targetDate } : null
          };
          ...
          result = await supabase
            ...
            .insert([{
              sector: 'proveedores',
              tipo: 'pedido_fabrica',
              producto: `Stock de Mercadería - Turno ${stockTurno}`,
              responsable: stockResponsable.trim(),
              fecha: targetDate,
              datos: initialDatos
            }])
        ...
        const { data: newRecs, error } = result;

        if (!error && newRecs) {
          await logAction(supabase, 'proveedores', 'Carga de Stock', stockResponsable.trim(), { 
            articulos: stockList.length 
          });

          await upsertNcNotification({
            targetSector: 'proveedores',
            emitterName: 'Proveedores',
            message: `NUEVA CARGA DE STOCK`,
            details: `Se ha registrado una carga de stock por ${stockResponsable.trim()}.`,
            refId: newRecs[0].id
          });
          ...
          alert('Carga de stock registrada correctamente en el historial de Proveedores y Compras.');
          setStockResponsable('');
          setActiveSubTab('history');
        } else if (error) {
          alert('Error al registrar la carga de stock: ' + error.message);
        }
  ...
  const handleAddNewProduct = () => {
    if (!newProductData.producto.trim()) {
    ...
    const newProd = {
      ...
      producto: newProductData.producto.trim().toUpperCase(),
      almacenamiento: newProductData.almacenamiento.trim().toUpperCase() || 'ALMACÉN',
      min: Number(newProductData.min) || 0,
      stock: Number(newProductData.stock) || 0,
      unidad: newProductData.unidad,
      vencimiento: newProductData.vencimiento.trim(),
      estado: newProductData.vencimiento.trim().toLowerCase() === 'n/a' ? 'OK VTO' : 'OK VTO'
    };
    setStockList(prev => [...prev, newProd]);
  ...
  const handleDeleteProduct = (id, name) => {
    if (confirm(`¿Está seguro de que desea eliminar el producto "${name}" del stock?`)) {
      setStockList(prev => prev.filter(item => item.id !== id));
    }
  };

  const handleSavePedidoMateriales = async () => {
    const validItems = pedidoItems.filter(item => item.producto.trim() && item.cantidad);
    if (validItems.length === 0) {
      alert('Por favor ingrese al menos un insumo con cantidad.');
      return;
    }
    const responsible = localStorage.getItem('regsapp_responsible') || 'Desconocido';
    const codigo = `PED-${activeSector.substring(0,3).toUpperCase()}-${Date.now().toString().slice(-6)}`;
    const recordData = {
      sector: activeSector,
      tipo: 'pedido_materiales',
      producto: 'PEDIDO A PROVEEDORES',
      responsable: responsible,
      fecha: formatInputDate(pedidoFecha),
      codigo: codigo,
      datos: {
        sector_creador: activeSector,
    ...
    setConfirmModal({
      ...
      action: async () => {
        const { data, error } = await supabase.from('registros').insert([recordData]).select();
        if (error) {
          alert('No se pudo guardar el pedido: ' + error.message);
        } else {
          alert('Pedido de materiales enviado correctamente.');
          setPedidoItems([{ producto: '', cantidad: '', unidad: 'KG' }]);
        }
        setConfirmModal({ show: false, action: null, title: '' });
      }
    });
...
const copyReportForEmail = (record) => {
    ...
    if (rType === 'report') {
      ...
      `;
    } else if (rType === 'pedido_fabrica') {
      let recordManana = record.datos?.manana;
      let recordTarde = record.datos?.tarde;

      if (!recordManana && !recordTarde && record.datos?.stockList) {
        const oldTurno = record.datos.turno || 'Mañana';
        const mockShiftObj = {
          stockList: record.datos.stockList,
          responsable: record.responsable || record.datos.responsable || 'Sistema',
          fechaCarga: record.fecha
        };
        if (oldTurno === 'Mañana') {
          recordManana = mockShiftObj;
        } else {
          recordTarde = mockShiftObj;
        }
      }

      if (!copyEmailManana && !copyEmailTarde) {
        alert('Por favor, seleccione al menos un turno para copiar.');
        return;
      }

      html += `
        <div style="margin-bottom: 15px; padding: 10px; background: #fffdf5; border: 1px solid #fef3c7; border-radius: 4px;">
          <strong>Control de Stock consolidado para el día:</strong> ${record.fecha || '-'}
        </div>
      `;

      if (recordManana && copyEmailManana) {
        html += `
          <h3 style="margin-top: 15px; margin-bottom: 5px; color: #333;">☀️ Turno Mañana (Responsable: ${recordManana.responsable})</h3>
          <table style="width: 100%; border-collapse: collapse; font-size: 13px; margin-bottom: 15px;">
            <thead>
              <tr style="background: #f3f4f6;">
                <th style="padding: 8px; border: 1px solid #ddd; text-align: left;">Materia prima</th>
                <th style="padding: 8px; border: 1px solid #ddd; text-align: center; width: 60px;">Stock</th>
                <th style="padding: 8px; border: 1px solid #ddd; text-align: center; width: 50px;">Unidad</th>
              </tr>
            </thead>
            <tbody>
              ${(recordManana.stockList || []).map(item => `
                <tr>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.producto}</td>
                  <td style="padding: 8px; border: 1px solid #ddd; text-align: center; ${Number(item.stock) < Number(item.min) ? 'color: red; font-weight: bold;' : ''}">${item.stock}</td>
      ...
      if (recordTarde && copyEmailTarde) {
        html += `
          <h3 style="margin-top: 15px; margin-bottom: 5px; color: #333;">🌙 Turno Tarde (Responsable: ${recordTarde.responsable})</h3>
          <table style="width: 100%; border-collapse: collapse; font-size: 13px; margin-bottom: 15px;">
            <thead>
              <tr style="background: #f3f4f6;">
                <th style="padding: 8px; border: 1px solid #ddd; text-align: left;">Materia prima</th>
            ...
            <tbody>
              ${(recordTarde.stockList || []).map(item => `
                <tr>
                  <td style="padding: 8px; border: 1px solid #ddd;">${item.producto}</td>
                  <td style="padding: 8px; border: 1px solid #ddd; text-align: center; ${Number(item.stock) < Number(item.min) ? 'color: red; font-weight: bold;' : ''}">${item.stock}</td>
                  <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${item.unidad}</td>
      ...
      if (!recordManana && !recordTarde) {
        html += `<p style="font-style: italic; color: #666;">Sin turnos registrados para este día.</p>`;
      }
    }
    ...
    setConfirmModal({
      ...
      action: async () => {
        ...
        if (!error && newRecs) {
          ...
          let targetIds = [];
          let notifMsg = `NUEVO INFORME (${finalFormData.codigo || 'S/C'})`;
          
          if (activeSector === 'marketing' || activeSubTab === 'personal') {
            targetIds = activeSubTab === 'personal' ? ['rrhh'] : (Array.isArray(finalFormData.tipoPrueba) ? finalFormData.tipoPrueba : (finalFormData.tipoPrueba ? [finalFormData.tipoPrueba] : []));
            notifMsg = activeSubTab === 'personal' ? `NOVEDAD DE PERSONAL: ${finalFormData.producto}` : `AVISO DE MARKETING: ${finalFormData.producto}`;
          } else {
            // Para otros sectores (como Desarrollo), enviar al sector seleccionado en tipoPrueba o Calidad/Desarrollo
            if (Array.isArray(finalFormData.tipoPrueba)) {
              targetIds = finalFormData.tipoPrueba;
            } else if (finalFormData.tipoPrueba) {
              targetIds = [finalFormData.tipoPrueba];
            } else {
              targetIds = ['calidad', 'desarrollo'].filter(s => s !== activeSector);
            }
          }
          ...
          for (const targetSectorId of targetIds) {
            ...
            await upsertNcNotification({
              ...
              emitterName: SECTORS.find(s => s.id === activeSector)?.label || activeSector,
              message: notifMsg,
              details: finalFormData.justificacion || finalFormData.descripcionPrueba || finalFormData.producto,
              refId: newRecs[0].id
            });
          }
    ...
    setConfirmModal({
      ...
      action: async () => {
        ...
        if (!error && data && data.length > 0) {
          ...
          // Notificar a Calidad y Desarrollo sobre el nuevo ingreso de materias primas
          const targetSectors = ['calidad', 'desarrollo'].filter(s => s !== activeSector);
          for (const targetId of targetSectors) {
            await upsertNcNotification({
              targetSector: targetId,
              emitterName: SECTORS.find(s => s.id === activeSector)?.label || activeSector,
              message: `INGRESO DE MATERIAL: ${materialsData.proveedor || 'Proveedor'}`,
              details: `Insumos: ${materialsData.insumos.filter(i => i.trim()).join(', ')}`,
              refId: data[0].id
            });
          }
  ...
  const filteredRecords = records.filter(record => {
    ...
    // Show Stock de Mercadería in compras and produccion
    if ((record.tipo === 'pedido_fabrica' || record.type === 'pedido_fabrica') && 
        (activeSector === 'compras' || activeSector === 'produccion')) {
      return true;
    }
  ...
  const filteredPedidos = records.filter(record => {
    return record.tipo === 'pedido_materiales' && record.fecha === formatInputDate(selectedHistoryDate);
  });
  ...
  const LogsView = ({ logs, onRefresh }) => (
    ...
    {(() => {
      ...
      return (
        ...
        >
          <History size={24} />
          <span className="sub-tab-label">PEDIDOS A PROVEEDORES</span>
                  ...
                  {(showTools ? TOOLS : SECTORS).map((item, index) => (
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        else if (item.id === 'produccion') defaultTab = 'novedades-turno';
                        else if (item.id === 'compras') defaultTab = 'stock-mercaderia-history';
                        else if (['mesa-carnes', 'cocina', 'picadillo', 'armado', 'salsas'].includes(item.id)) defaultTab = 'pedido-materiales';
          ...
          >
            <div style={{ padding: activeSector === 'sistemas' ? '0 0 1rem 0' : '0 0 4rem 0', minHeight: '100%' }}>
              <header className="header" style={activeSector === 'sistemas' ? { padding: '1rem 2rem' } : {}}>
                                        ...
                                        <div 
                                          ...
                                          onClick={() => {
                                            ...
                                            if (notif.type === 'novedades_turno' || notif.tipo === 'novedades_turno') {
                                              ...
                                              setActiveSubTab('pedido-materiales');
                                            } else if (refId && record) {
                ...
                {activeSubTab !== 'logs' && !PEDIDO_SECTORS.includes(activeSector) && (activeSector === 'calidad' || activeSector === 'desarrollo') && (
            ...
            {activeSector === 'calidad' ? (
              ...
              <>
                {(() => {
                  ...
                  return (
                    <>
                      {!['mesa-carnes', 'cocina', 'picadillo', 'armado', 'salsas', 'compras'].includes(activeSector) && (
                        <button 
                          onClick={() => { setActiveSubTab('form'); setSelectedRecord(null); }}
                          className={`sub-tab-btn ${activeSubTab === 'form' ? 'active' : ''}`}
                          style={btnStyle}
                      ...
                      {(activeSector !== 'calidad' && activeSector !== 'desarrollo' && !['mesa-carnes', 'cocina', 'picadillo', 'armado', 'salsas'].includes(activeSector)) && (
                        <button 
                          onClick={() => { setActiveSubTab('history'); setSelectedRecord(null); }}
                          className={`sub-tab-btn ${activeSubTab === 'history' ? 'active' : ''}`}
                          style={btnStyle}
                        >
                          ...
                          <span className="sub-tab-label">
                            {activeSector === 'proveedores' ? 'Historial Stock mercaderia' : 'VER HISTORIAL'}
                          </span>
                          {activeSubTab === 'history' && <motion.div layoutId="active-pill" className="sub-tab-active-bg" />}
                        </button>
                      )}

                      {['mesa-carnes', 'cocina', 'picadillo', 'armado', 'salsas', 'logistica'].includes(activeSector) && (
                        <button 
                          onClick={() => { setActiveSubTab('pedido-materiales'); setSelectedRecord(null); }}
                          className={`sub-tab-btn ${activeSubTab === 'pedido-materiales' ? 'active' : ''}`}
                          style={btnStyle}
                      ...
                      {['mesa-carnes', 'cocina', 'picadillo', 'armado', 'salsas'].includes(activeSector) && (
                        <button 
                          onClick={() => { setActiveSubTab('novedades-turno'); setSelectedRecord(null); }}
                          className={`sub-tab-btn ${activeSubTab === 'novedades-turno' ? 'active' : ''}`}
                          style={btnStyle}
          ...
          <AnimatePresence mode="wait">
            {selectedRecord ? (
                      ...
                      <div className="report-view-header">
                        <div className="header-main">
                          <h2>{(selectedRecord.sector === 'rrhh' || (selectedRecord.codigo && selectedRecord.codigo.startsWith('PERS-'))) ? 'Detalle de gestión del personal' : 
                               ...
                               (selectedRecord.type === 'pedido_fabrica' || selectedRecord.tipo === 'pedido_fabrica') ? 'Detalle de Stock de Mercadería' :
                      ...
                      <div className="report-view-footer">
                        <div className="view-group">
                          ...
                          <div className="view-signature">{selectedRecord.ingresadoPor}</div>
                              ...
                              ) : (selectedRecord.type === 'pedido_materiales' || selectedRecord.tipo === 'pedido_materiales') ? (
              <>
                <div className="report-view-header">
                  <div className="header-main">
                    <h2>Pedido de Materiales a Proveedores</h2>
                    <div className="type-indicator-bubble" style={{ background: '#ef4444' }}>PEDIDO DE PLANTA</div>
                  </div>
                  <div className="header-meta">
                ...
                <div className="report-view-grid">
                  <div className="view-group">
                    <label>Sector Emisor</label>
                    <div className="view-value">{SECTORS.find(s => s.id === selectedRecord.sector)?.label || selectedRecord.sector}</div>
                  </div>
                  <div className="view-group">
                ...
                <div className="report-view-grid">
                  <div className="view-group">
                    ...
                    <div className="view-value highlight">{selectedRecord.responsable}</div>
                  </div>
                  <div className="view-group">
                    <label>Artículos Pedidos</label>
                    <div className="view-value">{(selectedRecord.datos?.items || []).length} items</div>
                  </div>
                </div>

                <div className="view-group full" style={{ marginTop: '1.5rem' }}>
                  <label>Detalle de Insumos Solicitados</label>
                  <div className="view-value-table">
                    <div className="table-header-mini" style={{ display: 'grid', gridTemplateColumns: '3fr 1.5fr 1.5fr', fontWeight: 'bold', borderBottom: '2px solid rgba(255,255,255,0.1)', paddingBottom: '8px', marginBottom: '8px', fontSize: '0.75rem', textTransform: 'uppercase' }}>
                      <span>Producto / Materia Prima</span>
                      <span style={{ textAlign: 'center' }}>Cantidad</span>
                      <span>Unidad</span>
                    </div>
                    {(selectedRecord.datos?.items || []).map((item, idx) => (
                      <div key={idx} className="table-row-mini" style={{ display: 'grid', gridTemplateColumns: '3fr 1.5fr 1.5fr', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '0.8rem', alignItems: 'center' }}>
                        <span style={{ fontWeight: '500', color: '#fff' }}>{item.producto}</span>
                        <span style={{ textAlign: 'center', color: '#ef4444', fontWeight: 'bold' }}>{item.cantidad}</span>
                        <span style={{ opacity: 0.7 }}>{item.unidad}</span>
                      </div>
                    ))}
                  </div>
                </div>
              </>
            ) : (selectedRecord.type === 'pedido_fabrica' || selectedRecord.tipo === 'pedido_fabrica') ? (
              (() => {
                let recordManana = selectedRecord.datos?.manana;
                let recordTarde = selectedRecord.datos?.tarde;

                if (!recordManana && !recordTarde && selectedRecord.datos?.stockList) {
                  const oldTurno = selectedRecord.datos.turno || 'Mañana';
                  const mockShiftObj = {
                    stockList: selectedRecord.datos.stockList,
                    responsable: selectedRecord.responsable || selectedRecord.datos.responsable || 'Sistema',
                    fechaCarga: selectedRecord.fecha
                  };
                  if (oldTurno === 'Mañana') {
                    recordManana = mockShiftObj;
                  } else {
                ...
                const isDoubleTurn = recordManana && recordTarde;

                return (
                  <>
                    <div className="excel-container" style={{ margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
                      <div className="excel-header-branding">
                        ...
                        <div className="branding-center">
                          <h1>STOCKS MATERIAS PRIMAS {isDoubleTurn ? '(DOBLE TURNO)' : ''}</h1>
                        </div>
                        <div className="branding-right">
                          <div className="control-date-box">
                            <span className="label">FECHA DE CONTROL:</span>
                            <span className="date">{formatInputDate(selectedRecord.fecha)}</span>
                          </div>
                          <div style={{ fontSize: '0.75rem', opacity: 0.8, color: '#666', fontWeight: 'bold', marginTop: '4px', textAlign: 'right' }}>
                            {recordManana && recordTarde ? (
                              <>
                                RESP. MAÑANA: <span style={{ color: '#000' }}>{recordManana.responsable}</span> | RESP. TARDE: <span style={{ color: '#000' }}>{recordTarde.responsable}</span>
                              </>
                            ) : recordManana ? (
                              <>
                                RESPONSABLE MAÑANA: <span style={{ color: '#000' }}>{recordManana.responsable}</span>
                              </>
                            ) : recordTarde ? (
                              <>
                                RESPONSABLE TARDE: <span style={{ color: '#000' }}>{recordTarde.responsable}</span>
                              </>
                            ) : (
                              <>
                                RESPONSABLE: <span style={{ color: '#000' }}>{selectedRecord.responsable}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="excel-controls" style={{ marginBottom: '1rem', display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                        <div className="search-box-excel" style={{ flex: 1, minWidth: '200px', position: 'relative' }}>
                          <Search size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#666' }} />
                          <input 
                             type="text" 
                             placeholder="Buscar por materia prima..." 
                             className="excel-search-input"
                             style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.5rem', borderRadius: '4px', border: '1px solid #ccc', outline: 'none', fontSize: '0.9rem', color: '#000', background: '#fff' }}
                             value={detailSearchTerm}
                             onChange={(e) => setDetailSearchTerm(e.target.value)}
                          />
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', background: '#f5f5f5', padding: '0.3rem 0.8rem', borderRadius: '4px', border: '1px solid #ccc', height: '38px' }}>
                          <label style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', color: '#000', cursor: 'pointer', fontWeight: 'bold', userSelect: 'none' }}>
                            <input 
                              type="checkbox" 
                              checked={copyEmailManana} 
                              onChange={(e) => setCopyEmailManana(e.target.checked)}
                          ...
                          <label style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', color: '#000', cursor: 'pointer', fontWeight: 'bold', userSelect: 'none' }}>
                            <input 
                              type="checkbox" 
                              checked={copyEmailTarde} 
                              onChange={(e) => setCopyEmailTarde(e.target.checked)}
                              style={{ cursor: 'pointer', width: '14px', height: '14px' }}
                            />
                            Tarde
                          </label>
                        ...
                        <button 
                          onClick={() => copyReportForEmail(selectedRecord)}
                      ...
                      <div className="excel-dual-layout" style={{ display: 'flex', gap: '1.5rem', width: '100%', boxSizing: 'border-box', overflowX: 'auto', paddingBottom: '1rem' }}>
                        
                        {/* TURNO MAÑANA */}
                        <div style={{ flex: 1, minWidth: '400px' }}>
                          <h3 style={{ color: '#000', fontSize: '0.9rem', fontWeight: 'bold', borderBottom: '2px solid #ccc', paddingBottom: '0.4rem', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                            ☀️ Turno Mañana {recordManana ? `(Cargado por: ${recordManana.responsable})` : '(Sin registrar)'}
                          </h3>
                          {recordManana ? (
                            <div className="excel-table-wrapper" style={{ maxHeight: '500px', overflowY: 'auto' }}>
                              <table className="excel-table" style={{ width: '100%' }}>
                                ...
                                <tbody>
                                  {(recordManana.stockList || []).filter(item => 
                                    !detailSearchTerm ||
                                    item.producto.toLowerCase().includes(detailSearchTerm.toLowerCase()) ||
                                    item.tipo.toLowerCase().includes(detailSearchTerm.toLowerCase())
                                  ).map((item, idx) => {
                                    const isBelowMin = Number(item.stock) < Number(item.min);
                                    const stockClass = isBelowMin ? 'below-min' : 'normal';
                                    return (
                                      <tr key={item.id || idx} className={`row-type-${item.tipo.toLowerCase()}`}>
                                        <td className="col-num-excel">{idx + 1}</td>
                                        <td className="col-producto" style={{ color: '#000', fontSize: '0.8rem', padding: '0.5rem' }}>{item.producto}</td>
                                        <td className={`col-stock ${stockClass}`} style={{ textAlign: 'center', fontSize: '0.8rem', padding: '0.5rem' }}>{item.stock}</td>
                                        <td className="col-unidad" style={{ textAlign: 'center', fontSize: '0.8rem', color: '#000', padding: '0.5rem' }}>{item.unidad}</td>
                                        <td className="col-vencimiento" style={{ textAlign: 'center', fontSize: '0.8rem', color: '#000', padding: '0.5rem' }}>{item.vencimiento}</td>
                                        <td className={`col-estado ${item.estado === 'VENCIDO' ? 'vencido' : (item.estado === 'POR VENCER' ? 'por-vencer' : 'ok')}`} style={{ textAlign: 'center', fontSize: '0.75rem', padding: '0.5rem' }}>{item.estado}</td>
                                      </tr>
                                    );
                                  })}
                                </tbody>
                            ...
                            <div className="empty-state" style={{ height: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: 'rgba(0,0,0,0.03)', borderRadius: '8px', border: '1px dashed rgba(0,0,0,0.1)', padding: '2rem' }}>
                              <Package size={36} style={{ color: '#aaa', marginBottom: '0.5rem' }} />
                              <p style={{ color: '#666', fontSize: '0.85rem' }}>No se registró carga para el Turno Mañana.</p>
                            </div>
                          )}
                        </div>
                        ...
                        <div style={{ flex: 1, minWidth: '400px' }}>
                          <h3 style={{ color: '#000', fontSize: '0.9rem', fontWeight: 'bold', borderBottom: '2px solid #ccc', paddingBottom: '0.4rem', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                            🌙 Turno Tarde {recordTarde ? `(Cargado por: ${recordTarde.responsable})` : '(Sin registrar)'}
                          </h3>
                          {recordTarde ? (
                            <div className="excel-table-wrapper" style={{ maxHeight: '500px', overflowY: 'auto' }}>
                              <table className="excel-table" style={{ width: '100%' }}>
                                ...
                                <tbody>
                                  {(recordTarde.stockList || []).filter(item => 
                                    !detailSearchTerm ||
                                    item.producto.toLowerCase().includes(detailSearchTerm.toLowerCase()) ||
                                    item.tipo.toLowerCase().includes(detailSearchTerm.toLowerCase())
                                  ).map((item, idx) => {
                            ...
                            <div className="empty-state" style={{ height: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: 'rgba(0,0,0,0.03)', borderRadius: '8px', border: '1px dashed rgba(0,0,0,0.1)', padding: '2rem' }}>
                              <Package size={36} style={{ color: '#aaa', marginBottom: '0.5rem' }} />
                              <p style={{ color: '#666', fontSize: '0.85rem' }}>No se registró carga para el Turno Tarde.</p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="excel-footer-legend" style={{ marginTop: '1rem' }}>
                        <div className="legend-item" style={{ color: '#000' }}><span className="status-box ok"></span> OK VTO</div>
                        <div className="legend-item" style={{ color: '#000' }}><span className="status-box por-vencer"></span> POR VENCER</div>
                        <div className="legend-item" style={{ color: '#000' }}><span className="status-box vencido"></span> VENCIDO</div>
                      </div>
                    </div>
                  </>
                );
              ...
              <motion.div
                ...
                style={{ width: '100%' }}
              >
                <NovedadesTemplate activeSector={activeSector} />
              </motion.div>
            ) : activeSubTab === 'pedido-materiales' ? (
              <motion.div
                key={`${activeSector}-pedido-materiales`}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.2 }}
                style={{ width: '100%' }}
              >
                <PedidoMateriales activeSector={activeSector} />
              </motion.div>
            ...
            ) : activeSubTab === 'stock-mercaderia-history' ? (
              <motion.div
                key={`${activeSector}-stock-history`}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.2 }}
              ...
              >
                ...
                {filteredStockHistoryRecords.length > 0 ? (
                  <div className="history-list">
                    {filteredStockHistoryRecords.map(record => (
                      <motion.div
                        key={record.id}
                      ...
                      >
                        <div className="item-info">
                          <div className="item-type-label" style={{ color: '#10b981' }}>
                            📦 STOCK DE MERCADERÍA - TURNO {record.datos?.turno?.toUpperCase() || 'MAÑANA'}
                          </div>
                          <h3>{record.producto || 'Carga de Stock'}</h3>
                          <div className="item-meta">
                            <p><Clock size={12} /> {formatInputDate(record.fecha)}</p>
            ...
            ) : activeSubTab === 'planificador-recorrido' ? (
              <motion.div
                key="planificador-recorrido"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.2 }}
              ...
              >
                <div className="excel-container">
                  <div className="excel-header-branding">
                    ...
                    <div className="branding-right">
                      <div className="control-date-box" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <span className="label">FECHA DE CONTROL:</span>
                        <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
                          <input
                            type="text"
                            placeholder="DD/MM/AAAA"
                            value={stockControlDate.includes('-') ? formatInputDate(stockControlDate) : stockControlDate}
                            onChange={(e) => {
                              const masked = handleDateMask(e.target.value);
                              if (masked.length === 10) {
                                const [d, m, y] = masked.split('/');
                                if (Number(d) >= 1 && Number(d) <= 31 && Number(m) >= 1 && Number(m) <= 12 && y.length === 4) {
                                  setStockControlDate(`${y}-${m}-${d}`);
                                  return;
                                }
                              }
                              setStockControlDate(masked);
                            }}
                            className="excel-search-input"
                            style={{
                          ...
                          />
                          <div style={{ position: 'absolute', right: '5px', top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center', pointerEvents: 'none' }}>
                            <Calendar size={16} color="#666" />
                          </div>
                          <input
                            type="date"
                            value={stockControlDate.includes('/') ? '' : stockControlDate}
                            onChange={(e) => setStockControlDate(e.target.value)}
                            style={{
                              position: 'absolute',
                              right: 0,
                              top: 0,
                  ...
                  <div className="excel-controls" style={{ marginBottom: '1rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    ...
                    {activeSector === 'proveedores' && (
                      <div className="stock-confirm-controls" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        <input 
                          type="text" 
                          placeholder="Responsable de carga..." 
                          className="excel-search-input"
                          style={{ 
                            padding: '0.6rem 1rem', 
                            borderRadius: '4px', 
                          ...
                          value={stockResponsable}
                          onChange={(e) => setStockResponsable(e.target.value)}
                        />
                        <select
                          value={stockTurno}
                          onChange={(e) => setStockTurno(e.target.value)}
                          className="excel-search-input"
                          style={{ 
                            padding: '0.6rem 1rem', 
                            borderRadius: '4px', 
                        ...
                        <button 
                          type="button"
                          onClick={handleStockConfirmCarga}
                          className="copy-email-btn"
                          style={{
                            padding: '0.6rem 1.2rem',
                            background: '#10b981',
                    ...
                    <table className="excel-table">
                      <thead style={{ position: 'sticky', top: 0, zIndex: 5 }}>
                        <tr>
                          ...
                          <th>ESTADO</th>
                          {activeSector === 'proveedores' && <th style={{ width: '40px' }}>Acción</th>}
                        </tr>
                      </thead>
                      <tbody>
                        {stockList.filter(item => 
                          ...
                          return (
                            <tr key={item.id} className={`row-type-${item.tipo.toLowerCase()}`}>
                              ...
                              {activeSector === 'proveedores' && (
                                <td style={{ textAlign: 'center', padding: '0.2rem', position: 'relative' }}>
                                  <button
                                    type="button"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setOpenStockMenuId(openStockMenuId === item.id ? null : item.id);
                                    }}
                                    style={{
                                      background: openStockMenuId === item.id ? 'rgba(255, 255, 255, 0.1)' : 'transparent',
                                      border: 'none',
                                      color: openStockMenuId === item.id ? '#fff' : '#888',
                                      opacity: openStockMenuId === item.id ? 1 : 0.6,
                                      cursor: 'pointer',
                                      display: 'inline-flex',
                                      alignItems: 'center',
                                      justifyContent: 'center',
                                  ...
                                  {openStockMenuId === item.id && (
                                    <>
                                      <div 
                                        style={{ position: 'fixed', inset: 0, zIndex: 99 }}
                                        onClick={() => setOpenStockMenuId(null)}
                                      />
                                      <div 
                                        style={{
                                          position: 'absolute',
                                      ...
                                      >
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            handleDeleteProduct(item.id, item.producto);
                                          }}
                                          style={{
                                            width: '100%',
                                            display: 'flex',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '0.85rem', color: '#aaa' }}>Filtrar por fecha:</span>
                    <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
                      <input
                        type="text"
                        placeholder="DD/MM/AAAA"
                        value={historyFilterDate.includes('-') ? formatInputDate(historyFilterDate) : historyFilterDate}
                        onChange={(e) => {
                          const masked = handleDateMask(e.target.value);
                          if (masked.length === 10) {
                            const [d, m, y] = masked.split('/');
                            if (Number(d) >= 1 && Number(d) <= 31 && Number(m) >= 1 && Number(m) <= 12 && y.length === 4) {
                              setHistoryFilterDate(`${y}-${m}-${d}`);
                              return;
                            }
                          }
                          setHistoryFilterDate(masked);
                        }}
                        className="excel-search-input"
                        style={{
                      ...
                      <input
                        type="date"
                        value={historyFilterDate.includes('/') ? '' : historyFilterDate}
                        onChange={(e) => setHistoryFilterDate(e.target.value)}
                        style={{
                          position: 'absolute',
                          right: 0,
                ...
                {(() => {
                  const displayed = filteredRecords.filter(record => {
                    if (!historyFilterDate) return true;
                    const recordDate = record.fecha || record.fechaIngreso || record.created_at?.split('T')[0];
                    return formatInputDate(recordDate) === formatInputDate(historyFilterDate);
                  });
                  ...
                  return displayed.length > 0 ? (
                          ...
                          <div className="item-info">
                            <div className="item-type-label">
                              {(record.codigo && record.codigo.startsWith('PERS-')) ? '👤 REGISTRO DE PERSONAL' :
                               ...
                               (record.type === 'pedido_fabrica' || record.tipo === 'pedido_fabrica') ? ('📦 STOCK DE MERCADERÍA - TURNO ' + ((record.datos?.manana && record.datos?.tarde) ? 'MAÑANA Y TARDE' : record.datos?.manana ? 'MAÑANA' : record.datos?.tarde ? 'TARDE' : 'NO DEFINIDO')) :
                               '⚠️ NO CONFORMIDAD'}
                            </div>
                            ...
                            <div className="item-meta">
                              ...
                              ) : (record.type === 'pedido_fabrica' || record.tipo === 'pedido_fabrica') ? (
                                <span className="badge aprobado">
                                  CONTROLADO
                                </span>
                              ) : (
                    ...
                    <div className="empty-state">
                      <ClipboardList size={48} />
                      <p>{historyFilterDate ? `Sin registros para la fecha ${formatInputDate(historyFilterDate)}` : `Sin informes en ${SECTORS.find(s => s.id === activeSector)?.label || 'este sector'}`}</p>
                    </div>
                  );
                })()}
              </motion.div>
    ...
    <AnimatePresence>
      ...
      {newProductModal && (
        ...
        >
          ...
          >
            <div className="modal-icon" style={{ background: 'rgba(16, 185, 129, 0.1)', color: '#10b981' }}>
              <PlusCircle size={28} />
            </div>
            <h2 style={{ fontSize: '1.25rem', fontWeight: '800', textAlign: 'center', marginBottom: '1.5rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#fff' }}>
              Agregar Materia Prima
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%', boxSizing: 'border-box' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>TIPO / CATEGORÍA</label>
                <select
                  value={newProductData.tipo}
                  onChange={(e) => setNewProductData({...newProductData, tipo: e.target.value})}
                  style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #444', background: '#2a2a2a', color: '#fff', outline: 'none' }}
                >
                  {['VERDURA', 'CARNES', 'QUESOS', 'LACTEOS', 'HUEVOS', 'FIAMBRES', 'SECOS', 'CONGELADOS'].map(t => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>NOMBRE DE MATERIA PRIMA</label>
                <input
                  type="text"
                  placeholder="Ej: CEBOLLA BLANCA"
                  value={newProductData.producto}
                  onChange={(e) => setNewProductData({...newProductData, producto: e.target.value.toUpperCase()})}
                  style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #444', background: '#2a2a2a', color: '#fff', outline: 'none', boxSizing: 'border-box' }}
                />
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>ALMACENAMIENTO</label>
                <input
                  type="text"
                  placeholder="Ej: CAMARA 5"
                  value={newProductData.almacenamiento}
                  onChange={(e) => setNewProductData({...newProductData, almacenamiento: e.target.value.toUpperCase()})}
                  style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #444', background: '#2a2a2a', color: '#fff', outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>STOCK MÍNIMO</label>
                  <input
                    type="number"
                    value={newProductData.min}
                    onChange={(e) => setNewProductData({...newProductData, min: parseFloat(e.target.value) || 0})}
                    style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #444', background: '#2a2a2a', color: '#fff', outline: 'none', textAlign: 'center', boxSizing: 'border-box' }}
                  />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>STOCK ACTUAL</label>
                  <input
                    type="number"
                    value={newProductData.stock}
                    onChange={(e) => setNewProductData({...newProductData, stock: parseFloat(e.target.value) || 0})}
                    style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #444', background: '#2a2a2a', color: '#fff', outline: 'none', textAlign: 'center', boxSizing: 'border-box' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>UNIDAD DE MEDIDA</label>
                  <select
                    value={newProductData.unidad}
                    onChange={(e) => setNewProductData({...newProductData, unidad: e.target.value})}
                    style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #444', background: '#2a2a2a', color: '#fff', outline: 'none' }}
                  >
                    {['KG', 'UNIDADES', 'CAJAS', 'MANGAS', 'BOLSAS'].map(u => (
                      <option key={u} value={u}>{u}</option>
                    ))}
                  </select>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#aaa', textAlign: 'left' }}>FECHA VENCIMIENTO</label>
                  <input
                    type="text"
                    placeholder="Ej: n/a o DD/MM/YYYY"
            ...
            <div className="modal-actions" style={{ marginTop: '2rem', display: 'flex', gap: '1rem', width: '100%' }}>
              <button 
                type="button"
                className="modal-btn cancel" 
                onClick={() => {
              ...
              <button 
                ...
                style={{ flex: 1, background: '#10b981', margin: 0 }}
              >
                Agregar
              </button>
            </div>
